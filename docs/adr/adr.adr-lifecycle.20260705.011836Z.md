...
# ADR: ADR Lifecycle and Naming Consolidation
...
| Proposal ADRs use `dev/<proposal-id>/` as candidate/review packet surfaces | `docs/adr/adr.adr-lifecycle-promotion-mechanics.md`; `docs/architecture/architecture.lifecycle.00.md` | Preserved, with legacy `proposed` and `active review` terminology mapped to `proposal` or `draft` by completeness. |
...
- current schema-backed practice uses `proposal`, `draft`, `accepted`, `active`, and `superseded` as record statuses
- older lifecycle drafts used `proposed`, `completed`, `rejected`, and `historical` language that must be reconciled after the current lifecycle update
- promotion mechanics preserve proposal, accepted, active, and superseded traceability
...
The following source elements are explicitly out of scope for this ADR: lifecycle state ownership by role, required `proposal` or `draft` sections, optional workflow gate fields, deprecated `docs/incubator/` and `docs/spikes/` migration, and the broader legacy-source index flow `idea -> spike -> proposal -> draft ADR -> accepted ADR -> active ADR -> implementation brief -> iterative implementation`. Those elements MUST NOT become changed authority without separate acceptance or a separate documentation handoff.
...
ADR records governed by this ADR MUST use the following canonical status vocabulary, aligned with the current schema surface:
...
`proposal` MUST mean a candidate idea or review packet that is not yet a complete ADR. `draft` MUST mean a complete ADR review record that is not accepted authority. `accepted` MUST mean that the decision is approved document authority but not necessarily current for enforcement or routing. `active` MUST mean accepted and current controlling authority for work, enforcement, routing, or document interpretation. Live work-in-progress state MAY use workspace control surfaces such as `workspaces/<role>/active.md`; it MUST NOT be confused with ADR status `active`.

### Compatibility and migration contract

Existing ADRs and schema-backed records with `status: accepted` or `status: active` MUST remain valid. Acceptance of this lifecycle update MUST NOT invalidate recent accepted ADR metadata or the `docs/schemas/schema.record-base.json` `RecordStatus` enum.

Compatibility terms from older drafts map as follows:

| Older term | Canonical status or meaning | Migration rule |
|---|---|---|
| `proposed` | `proposal` for candidate/review packets; `draft` when the document is complete enough for ADR review | Implementations MUST migrate machine statuses away from `proposed`; prose may retain it only as historical wording. |
| `completed` | `active` when the decision remains controlling after rollout; `superseded` when the completed work is no longer current | Implementations MUST NOT use this as a new record status. |
| `rejected` | review outcome/provenance, not canonical ADR status | Implementations MUST preserve rejection evidence as provenance or mark the durable record `superseded` when a durable record must remain. |
| `historical` | umbrella description for no-longer-current records, usually `superseded` | Implementations MUST NOT use this as a new record status unless a future schema migration explicitly adds it. |
| active review | `proposal`, `draft`, or workspace-local `active.md` | Implementations MUST NOT confuse workspace-local `active.md` with ADR status `active`. |
...
- A `proposal` ADR MUST be the candidate/review packet and SHOULD be located under `dev/<proposal-id>/`.
- A `draft` ADR MUST be complete enough for ADR review.
- An `accepted` ADR MUST be approved document authority and SHOULD be located under `docs/adr/`.
- An `active` ADR MUST be the current controlling authority for its governed surface.
- Superseded records and rejected outcomes MUST remain available as trace.
...
ADR titles and Markdown headings MUST NOT carry lifecycle timestamps. Timestamps belong in leading JSON metadata/provenance blocks. This ADR MUST NOT establish detailed filename rules for other ADRs. The older draft filename model `adr.<name>.md` / `adr.<name>.<status>.md` remains non-canonical draft guidance until a separate naming-rule reconciliation accepts, revises, or supersedes it. Metadata timestamps SHOULD use the existing schema-compatible forms `YYYYMMDD.HHMMSS` or `YYYYMMDD.HHMMSSZ`; timezone suffix forms such as `PST` are legacy/proposal-local and SHOULD NOT be introduced as new canonical machine-parseable formats.
...
- A reviewer can distinguish `proposal` candidate state, `draft` review state, `accepted` document authority, and `active` controlling authority.
- A reviewer can identify how legacy `proposed`/`completed`/`rejected`/`historical` wording maps to current `proposal`/`draft`/`accepted`/`active`/`superseded` vocabulary.
...
- The consolidation MUST distinguish ADR status `accepted` from ADR status `active`.
- The consolidation MUST NOT treat proposal, draft review state, or workspace `active.md` state as active ADR authority.
- Superseded records and rejected outcomes MUST remain trace, not dead files.
...
- This ADR MUST NOT change ADR JSON schemas without an explicit user-directed schema update.
