# Requirements draft 20260711.091607Z: workflow object from AAR synthesis

## Metadata

- Type: requirements-draft
- Status: candidate-requirements-not-authority
- Repository: projectkoios-bootstrap
- Owner: KOIOS
- Source consolidation: `docs/process-capture/pc.aar-consolidation.20260711.091607Z.md`
- Source count: 298 AAR files
- Authority: non-authoritative requirements candidate

## Scope

This draft massages consolidated AAR observations into candidate requirements for
a later workflow object. It is intended for ATHENA/user review before any
implementation. It does not authorize implementation, schema design, workflow
policy, storage choices, or role changes.

The candidate workflow object would represent bounded work as document-state
movement across role-owned artifacts, validation gates, provenance evidence, and
explicit non-authority boundaries.

## Source AAR index

Every AAR listed here was included in the synthesis.
...
1. Durable repository artifacts are the only reliable cross-session workflow
   state.
2. Roles and artifact domains must be explicit to prevent silent authority
   promotion.
3. Bounded slices with named source artifacts, plans, approval gates, validation,
   reports, and reviews produce safer handoffs than broad ambiguous tasks.
4. Chat/intercom routing is operationally useful but insufficient as durable
   authority.
5. Validation evidence must record command, working directory, scope, result, and
   limits.
6. Dirty-tree and generated-file state must be classified before packaging.
7. Draft/proposal/incubator/generated/fixture/sidecar statuses need explicit
   lifecycle fields.
8. Fixture and sidecar provenance are part of the review object when source
   fields are omitted from canonical schemas.
9. User-facing UI/process artifacts need preview/inspection evidence in addition
   to automated validation.
10. Tooling and dependency choices should remain local and draft until promoted
   into accepted policy or architecture.

## Candidate workflow-object requirements

Each requirement is a candidate only. Items marked **Needs ATHENA** require
architecture/spec promotion before implementation.

### R1. Work item identity and source packet — Needs ATHENA

A workflow object SHOULD have a stable work item id, title, initiating request or
source packet, source artifact paths, represented role, repository, workspace,
and creation timestamp.

Evidence basis: repeated session-start, handoff, ADR, and implementation AARs
where later agents needed durable source context.

### R2. Artifact node model — Needs ATHENA

A workflow object SHOULD model role-owned artifact nodes with at least: path,
artifact type, owner role, status, authority boundary, source hash when useful,
created/updated timestamp, and backward links to prior artifacts.

Evidence basis: ADR lifecycle, architecture index, workspace-state, template,
ADR JSON, and process-capture AARs.

### R3. Transition/gate model — Needs ATHENA

A workflow object SHOULD model transitions such as intake, brief, plan, approval,
implementation, validation, report, conformance review, provenance audit,
preview, packaging, and closeout.

Each transition SHOULD record consumed artifacts, produced artifacts, actor role,
explicit approval source if any, and pause/blocker state.

### R4. Role-domain guard — Needs ATHENA

The object SHOULD prevent or flag transitions where one role's artifact silently
creates another domain's authority, for example implementation convenience
creating architecture authority or process-capture text becoming policy.

### R5. Approval and pause state — Needs ATHENA

The object SHOULD represent approval-required, paused, approved, rejected,
implemented, validated, reviewed, and superseded states separately from file
existence.

### R6. Validation evidence record — Needs ATHENA/VULCAN

Validation evidence SHOULD include command, working directory, target scope,
stdout/stderr summary, pass/fail/non-applicable result, validator limitations,
timestamp, and source report path.

VULCAN should own implementation-level command validation details if promoted.

### R7. Dirty-tree/package boundary record — Needs ATHENA/VULCAN

Before commit/closeout, the object SHOULD classify changed paths by owner/slice,
mark generated/local-only files, identify unrelated dirty state, and record
staging/commit boundaries.

### R8. Non-authority and lifecycle markers — Needs ATHENA

The object SHOULD explicitly support statuses such as draft, proposal,
incubating, fixture-only, generated, sidecar, archived, superseded,
non-authoritative, accepted, active, and implemented-without-architecture-promotion.

### R9. Fixture/sidecar provenance record — Needs ATHENA

For fixture-backed, generated, or schema-conformance artifacts, the object SHOULD
track source path, source hash, generated artifact hash, omitted fields,
sidecar-preserved facts, transformation notes, freshness/stale markers, and trust
or authority notes.

### R10. User-preview validation record — Needs ATHENA

For UI/operator-facing surfaces, the object SHOULD be able to record preview URL
or preview method, inspected surface, user-visible question being validated,
observed user feedback, and whether the feedback changed scope.

### R11. Ephemeral-message promotion rule — Needs ATHENA/HERMES

If chat/intercom/user clarification materially changes acceptance criteria, the
object SHOULD require a durable artifact addendum or explicit provenance note
before final implementation/report packaging.

HERMES may own orchestration policy if promoted.

### R12. Dependency/tooling decision record — Needs ATHENA/VULCAN

The object SHOULD record whether tooling/dependencies are package-local,
optional, draft policy, accepted policy, or product architecture. It SHOULD flag
license/security/audit blockers separately from implementation failures.

### R13. Skill/reusable-procedure stability record — Needs ATHENA/KOIOS/VULCAN

A reusable skill/procedure SHOULD include status, validation evidence, owning
review, scope, and promotion state. File presence alone MUST NOT imply stable
skill authority.

### R14. Process-capture observation link — KOIOS-owned candidate

The object MAY link to KOIOS process-capture observations and AAR synthesis, but
those links MUST remain advisory unless promoted by another owner.

## Evidence/source mapping

| Requirement | Main source theme(s) | Representative AAR references |
|---|---|---|
| R1 | startup, handoff, source packets | `aar.20260701.014653_session-start-state-check.md`, `aar.20260702.020601_canonical-workspace-state-protocol.md`, `aar.20260708.041331_template-representation-vulcan-handoff.md` |
| R2 | document-state artifacts | `aar.20260701.122628_docs-architecture-canonicalization.md`, `aar.20260705.111255_workspace-adr-consolidation.md`, `aar.20260704.091823_koios-process-capture-and-docs-cleanup.md` |
| R3 | slice transitions | `aar.20260705.102506_workflow-petri-net-executor-first-slice.md`, `aar.20260708.044531_template-representation-roundtrip.md`, `aar.20260711.065704_json-schemas-adr-conformance.md` |
| R4 | authority guard | `aar.20260701.014145_promotion-review-routing.md`, `aar.20260702.023544_koios-comment-attribution-correction.md`, `aar.20260705.142149_petrinet-separation-adr-remediation.md` |
| R5 | approval/pause | `aar.20260702.205545_prompt-iterate-vulcan-blocker-handling.md`, `aar.20260709.014124_adr-json-database-pilot-brief.md`, `aar.20260711.035759_adr-json-database-one-adr-pilot.md` |
| R6 | validation evidence | `aar.20260704.193035_python-policy-validator-first-slice.md`, `aar.20260705.173808_petrinet-followups.md`, `aar.20260711.081405_operator-console-review-one-proposal-fixture.md` |
| R7 | dirty-tree/package boundary | `aar.20260701.053127_dirty-tree-review.md`, `aar.20260702.052145_blind-commit-all-scope.md`, `aar.20260705.101124_violation-formatting-test-policy-remediation.md` |
| R8 | lifecycle markers | `aar.20260702.184300_adr-lifecycle-converted-to-adr.md`, `aar.20260702.193846_lifecycle-status-and-promotion-model-refresh.md`, `aar.20260705.011110_adr-lifecycle-naming-consolidation-proposal.md` |
| R9 | fixture/sidecar provenance | `aar.20260711.035759_adr-json-database-one-adr-pilot.md`, `aar.20260711.065704_json-schemas-adr-conformance.md`, `aar.20260711.081405_operator-console-review-one-proposal-fixture.md` |
| R10 | user preview | `aar.20260711.081405_operator-console-review-one-proposal-fixture.md`, `aar.20260711.090601_operator-console-fixture-interaction-visibility.md` |
| R11 | ephemeral-message promotion | `aar.20260703.024352_mailbox-bridge-validation-friction.md`, `aar.20260709.012011_template-record-roundtrip-skill.md`, `aar.20260706.045501_workflow-adapter-contract-hardening.md` |
| R12 | tooling/dependency | `aar.20260701.030500_ollama-processing-visibility.md`, `aar.20260706.045501_workflow-adapter-contract-hardening.md`, `aar.20260711.081405_operator-console-review-one-proposal-fixture.md` |
| R13 | skills | `aar.20260709.010343_template-record-roundtrip-skill-brief.md`, `aar.20260709.010828_koios-comments-skill-brief-update.md`, `aar.20260709.012011_template-record-roundtrip-skill.md` |
| R14 | process capture | `aar.20260703.000000_graphrag-process-capture-observation.md`, `aar.20260704.091823_koios-process-capture-and-docs-cleanup.md` |

The full source index above is the coverage record for all AARs used; the table
here lists only representative evidence for each candidate requirement.

## Open questions / needs ATHENA decision

- Should the workflow object be a schema-backed JSON record, Markdown record, or
  both?
- Which artifact statuses and transition names are canonical versus local
  vocabulary?
- Which gates are mandatory for all slices and which are optional by artifact
  type?
- Should user-preview validation be a generic transition or a UI-specific
  transition?
- What is the relation between a future workflow object and existing `state.md`,
  `active.md`, ADRs, architecture notes, implementation reports, and AARs?
- Which owner may mark a work item complete when implementation, architecture,
  provenance, and packaging are all involved?

## Non-requirements / do-not-promote-yet

- Do not infer that every AAR observation must become workflow policy.
- Do not make KOIOS process-capture notes architecture authority.
- Do not require all historical AARs to be backfilled into workflow objects.
- Do not require live intercom/session integration for a first workflow object.
- Do not require database storage, Petri-net runtime execution, or UI console
  support as part of the requirements draft.
- Do not promote TypeScript DataObject/ActionObject or enum-class style lessons
  into global policy from this draft alone.

## Next recommended owner

- ATHENA: review and decide whether to promote this requirements draft into an
  architecture/spec surface for a workflow object.
- HERMES: coordinate if the workflow object changes cross-role orchestration or
  completion authority.
- VULCAN: wait for an accepted implementation brief before implementing schema,
  storage, CLI, or UI support.
- KOIOS: preserve additional AAR/process evidence and audit provenance if the
  draft is promoted.
