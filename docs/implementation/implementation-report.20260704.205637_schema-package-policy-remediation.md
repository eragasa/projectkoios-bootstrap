...
- Remediation target: `src/python/projectkoios/bootstrap/schemas/`
...
result=PythonPolicyValidator().validate_targets(TargetSelector(Path.cwd()).explicit_targets((Path('src/python/projectkoios/bootstrap/schemas'),)))
...
```bash
uv run mypy src/python/projectkoios/bootstrap/schemas
```
...
- This slice remediated only `src/python/projectkoios/bootstrap/schemas/` to keep changes reviewable.
