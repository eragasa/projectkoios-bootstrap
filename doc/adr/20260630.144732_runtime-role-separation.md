...
Draft — routed to Athena for review and acceptance.
...
The new koios-handoff-operator skill (being implemented under
`agents/global/pi/skills/`) and the handoff-ledger projection model both need
to reference role identity — which harness is responsible for what, which role
produced an artifact, which role should receive it.
...
- **Immediate:** The koios-handoff-operator skill scripts can import `Role`
...
- Do not change the filesystem layout of `agents/global/<name>/` or
  `~/.<runtime>/`. Those stay as-is.
...
This should be decided before the koios-handoff-operator skill scripts are
implemented, because those scripts will import from `models.py`. Making the
change now means the skill code targets the clean API. Making it after means
refactoring both the model and the new code.
