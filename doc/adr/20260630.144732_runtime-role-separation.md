...
This should be decided before the `archon_run_watch` skill scripts gain a
hard dependency on `models.py`. Making the change now means the skill code
targets the clean API. Making it after means refactoring both the model and
the new code.
