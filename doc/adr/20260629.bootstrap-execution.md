...
Created `doc/` with `adr/` subdirectory for future ADRs. Moved
`docs/architecture.00.md` → `doc/architecture.00.md`. Updated the deprecation
pointer in `architecture/harness-boundaries.md`.
...
- The `scripts/koios` bash script is now a thin Python wrapper. The old bash
  ...
  `harnesses` command.
- Future ADRs go in `doc/adr/` rather than `architecture/`.
