...
If not installed, direct the user to install Codex and stop.
...
- **No `.env` required**: CLI-only usage doesn't need any environment variables. If no API keys are in the environment, the CLI auto-defaults to global Codex auth from `codex login`.
