...
1. Petri-net architecture elaboration now has a single working synthesis/decomposition surface at `docs/architecture/architecture.petrinet.00.md`; update that document first until a section decomposes into a bounded ADR/spec/brief.
...
- Petri-net separation accepted ADR: `docs/adr/adr.petrinet.20260705.132740Z.md`; VULCAN implementation routing returned validated remediation; ATHENA conformance reviews completed. Current Petri-net architecture elaboration should be collected in `docs/architecture/architecture.petrinet.00.md` before decomposition.
...
- Explicit user direction before editing architecture notes outside the current bounded Athena architecture/specification scope.
