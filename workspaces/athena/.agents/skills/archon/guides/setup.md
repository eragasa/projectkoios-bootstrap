...
4. Check Codex is installed: `which codex`, then `codex login` if needed

> **Note — Codex binary path.** Archon does not bundle Codex. In compiled Archon binaries (quick install, Homebrew), the Codex CLI needs `CODEX_CLI_PATH` set to the absolute path of the executable. The `archon setup` wizard in Step 4 auto-detects this via `npm root -g` and writes it to `~/.archon/.env` — no manual action needed in the typical case. Source installs (`bun run`) don't need this; the CLI finds the executable via `node_modules` automatically.
...
> 2. AI assistant configuration (Codex and/or Claude)
...
| Auth errors | Codex not authenticated | Run `codex login` |
...
```
Header: "Archon skill"
Question: "Would you like to copy the Archon skill into your target repo so Codex can invoke Archon workflows from there?"
Options:
  1. "Yes, copy the skill" — Copies .agents/skills/archon/ into the target repo. This will appear in git unless you gitignore it.
...
2. `cd <target-repo>`
3. Run `codex` to launch Codex
4. The archon skill is now loaded — ask Codex to run workflows, fix issues, review PRs, etc.
...
- `.agents/skills/archon/` — skill for Codex integration
...
The end state: user is in their target repo with the Archon skill available, defaults loaded at runtime without polluting the repo, using Codex as the interface.
