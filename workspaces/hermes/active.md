...
1. Define a small non-ADR workflow-bootstrap pilot for `docs/plans/petrinet-projectmanagement.20260712.project-alignment.md`, reusing existing workflow/Petri-net code, preserving `petrinet -> workflow -> pm` separation, adding adapter-backed validation, and using the Operator Console fixture as the primary projection-only visibility surface, with interactive input deferred to later design.
