...
HERMES also captured USER initial alignment for a filesystem-backed Petri-net/Gantt project-management system in `docs/plans/petrinet-projectmanagement.20260712.project-alignment.md`; KOIOS reviewed it as provenance-adequate draft alignment/intake. USER later clarified that the system should be bootstrapped through the working process outside the ADR process, so the next recommended path is a bounded non-ADR workflow-bootstrap pilot rather than an ADR-first architecture route.
