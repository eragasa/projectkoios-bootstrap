...
def _token(
    ...
    """Create a handoff artifact fixture for implementation-owner guard tests."""
...
def test__Guards__wrong_implementation_owner__vulcan_impl_is_not_violation() -> None:
    ...
    token: HandoffArtifact = _token("valid-impl", kind="implementation-report", sender="Vulcan")
    # Marking places the fixture token into the handoff net.
    marking: HandoffMarking = Marking(tokens_by_place={"vulcan_inbox": [token]})
...
def test__Guards__wrong_implementation_owner__opencode_impl_is_not_violation() -> None:
    ...
    # Token is the implementation report fixture under guard evaluation.
    token: HandoffArtifact = _token("valid-impl", kind="implementation-report", sender="opencode")
    # Marking places the fixture token into the handoff net.
    marking: HandoffMarking = Marking(tokens_by_place={"vulcan_inbox": [token]})
    # Violations are the emitted guard failures under assertion.
    violations: list[Violation] = check_wrong_implementation_owner(marking)
...
def test__Guards__wrong_implementation_owner__hermes_impl_report_is_violation() -> None:
    ...
    # Token is the implementation report fixture under guard evaluation.
    token: HandoffArtifact = _token("hermes-impl", kind="implementation-report", sender="Hermes")
    # Marking places the fixture token into the handoff net.
...
def test__Guards__wrong_implementation_owner__hermes_patch_is_violation() -> None:
    ...
    token: HandoffArtifact = _token("hermes-patch", kind="patch", sender="Hermes")
    # Marking places the fixture token into the handoff net.
    marking: HandoffMarking = Marking(tokens_by_place={"pi_inbox": [token]})
    # Violations are the emitted guard failures under assertion.
    violations: list[Violation] = check_wrong_implementation_owner(marking)
...
def test__Guards__wrong_implementation_owner__athena_test_results_is_violation() -> None:
    ...
    token: HandoffArtifact = _token("athena-tests", kind="test-results", sender="Athena")
    # Marking places the fixture token into the handoff net.
