...
2. **Check current work** — inspect only ADRs actionable for Vulcan (Accepted
   ...
   Hermes/Athena scope unless Hermes sent a message into the Vulcan/opencode
   sandbox for you.
...
> section that Hermes has sent into the Vulcan/opencode sandbox, or a directive/handoff with consumer/recipient
> matching opencode/Vulcan. Draft ADRs (even those specifying Vulcan work) need
> a Hermes sandbox message delivery decision before implementation begins.
...
4. **Document decisions in ADRs** — if the session produced durable architectural decisions, update or create an ADR in `docs/adr/`.
